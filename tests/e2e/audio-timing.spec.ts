...
test.describe('Audio Timing and Synchronization', () => {
  ...
  test.skip('should maintain consistent BPM timing', async ({ page }) => {
    ...
    for (let i = 1; i < timings.length; i++) {
      const currentTiming = timings[Math.min(i, timings.length - 1)]
      const previousTiming = timings[Math.max(0, Math.min(i - 1, timings.length - 1))]
      if (currentTiming !== undefined && previousTiming !== undefined) {
        intervals.push(currentTiming - previousTiming)
      }
    }
  ...
  test('should handle drum pattern loops without timing drift', async ({ page }) => {
    ...
    for (let i = 1; i < loopTimings.length; i++) {
      const currentTiming = loopTimings[Math.min(i, loopTimings.length - 1)]
      const previousTiming = loopTimings[Math.max(0, Math.min(i - 1, loopTimings.length - 1))]
      if (currentTiming !== undefined && previousTiming !== undefined) {
        loopIntervals.push(currentTiming - previousTiming)
      }
    }
